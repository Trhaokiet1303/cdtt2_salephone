...
function loadCities() {
    fetch('https://vnprovinces.pythonanywhere.com/api/provinces/?basic=true&limit=100')
        ...
        .then(data => {
            ...
            citySelect.innerHTML = data.results.map(city => '<option value="' + city.id + '">' + city.full_name + '</option>').join('');
        })
        .catch(error => {
...
function updateDistricts() {
    ...
    console.log(cityId);
    fetch(`https://vnprovinces.pythonanywhere.com/api/provinces/${cityId}`) // Thay thế URL này bằng URL thực tế của bạn
        ...
        .then(data => {
            ...
            data.districts.forEach(district => {
                ...
                option.textContent = district.full_name; // Sử dụng thuộc tính full_name
...
function updateWards() {
    ...
    fetch(`https://vnprovinces.pythonanywhere.com/api/districts/${districtId}`) // Thay thế `api/wards/${districtId}` bằng URL thực tế của bạn
        .then(response => response.json())
        .then(data => {
            ...
            data.wards.forEach(ward => {
                ...
                option.textContent = ward.full_name; // Sử dụng thuộc tính full_name
...
function changeAddress(event) {
    event.preventDefault();
    //lấy phân nội dung được chọn trong các thẻ select
    ...
    //tạo một biến và công các giá trị thành một chuỗi
